...
const EcString plugin_name=cyton_bin_path + "cytonPlugin";

//acessing cytonConfig.xml
const EcString xml_name=cyton_bin_path + "cytonConfig.xml";
...

{
   ...
   passed &=hardware.setJointCommands(time,angles,cyton::JointAngleInRadians, jointRates);
   if(passed)
   {
...

{
   ...
   ros::init(argc,argv,"cyton_move_node");
   if(!hardware.init())
   {
